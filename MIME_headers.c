...
int MIMEH_parse_contenttype( char *header_name, char *header_value, struct MIMEH_header_info *hinfo )
{
	...
	p = PLD_strstr(header_name,"content-type",1);
	if (p != NULL)
	{
		DMIMEH LOGGER_log("%s:%d:MIMEH_parse_contenttype: Content-type string found in header-name",FL);
		DMIMEH LOGGER_log("%s:%d:MIMEH_parse_contenttype: Working string: '%s'",FL, header_value);
		...
		DMIMEH LOGGER_log("%s:%d:MIMEH_parse_contenttype:DEBUG: Content-type set to '%d'",FL, hinfo->content_type);
		...
		p = header_value;
		DMIMEH LOGGER_log("%s:%d:MIMEH_parse_contenttype:DEBUG: Searching for name=value pairs in '%s'", FL, p);
		if (p != NULL)
		{
			char *c = p;
		...
		if (p != NULL)
		{
			...
			while ( param != NULL )
			{
				...
				DMIMEH LOGGER_log("%s:%d:MIMEH_parse_contenttype:DEBUG: Boundary param<=>data_end gap = %d", FL,data_end_point -param);
				DMIMEH LOGGER_log("%s:%d:MIMEH_parse_contenttype:DEBUG: Boundary param start pos = '%s'",FL, param);
				if (data_end_point > param) param = data_end_point;
				DMIMEH LOGGER_log("%s:%d:MIMEH_parse_contenttype:DEBUG: Boundary param start pos = '%s'",FL, param);
...
//	if (tlen < 1) return 0; // PLD-20100501-1215; empty string premature exit optimisation.
...
int MIMEH_headers_process( struct MIMEH_header_info *hinfo, char *headers )
{
	...
	if (headerlength < 1) return 0; // PLD:20100501-1212; Optimisation if there's no data in the string.
	...
	DMIMEH LOGGER_log("%s:%d:MIMEH_headers_process:DEBUG: current_header_position = '%s'",FL,current_header_position);
	...
	while ((current_header_position != NULL)&&( current_header_position <= (h +headerlength) ))
	{
		...
		header_name = header_value = header_name_end_position = header_value_end_position = NULL;
		...
		while ((*current_header_position==' ')||(*current_header_position=='\t')) current_header_position++; // PLD-20100501-1549; eat leading whitespace.

		header_name = current_header_position;
		DMIMEH LOGGER_log("%s:%d:MIMEH_headers_process:DEBUG: Header_name position='%p'",FL, header_name);
		header_name_end_position = strpbrk( header_name, ":\t " );
		DMIMEH LOGGER_log("%s:%d:MIMEH_headers_process:DEBUG: End position of header is '%s'",FL,header_name_end_position);
