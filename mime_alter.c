...
int AM_read_headers(struct AM_disclaimer_details *dd, FFGET_FILE *f, FILE *newf )
{
	...
	dd->segment_start = FFGET_ftell(f); //20100301-2105-PLD
	...
	*textptr = NULL;
	...
	if (0 == stat(fname, &st))
	{
		...
		if (p)
		{
			...
			bc = fread(p, 1, st.st_size, f);
			if (bc != st.st_size) LOGGER_log("%s:%d:AM_disclaimer_load_text:ERROR: Read %d bytes instead of %d from %s", FL, bc, st.st_size, fname);
			//			*textptr[st.st_size] = '\0';
...
int AM_disclaimer_html_perform_insertion( struct AM_disclaimer_details *dd, FFGET_FILE *f, FILE *newf )
{
	...
	if (dd->content_encoding == _CTRANS_ENCODING_QP) {
		...
		qp_encode( qp_data, qp_data_size, data_to_use, strlen(data_to_use), glb.ldelimeter);
...
char *AM_insert_HTML_disclaimer_into_buffer( char *buffer, struct AM_disclaimer_details *dd )
{
	...
	if (glb.pretext_insert==1){
		/* HTML insertions are a bit annoying because of poorly formatted standards in email */
		p = PLD_strstr( buffer, "<body", 1);
		if (!p) p = PLD_strstr( buffer, "<html",1 );
		if (!p) p = strchr( buffer, '<' );

		if (p == NULL) {
			DAM  LOGGER_log("%s:%d:AM_insert_HTML_disclaimer_into_buffer:DEBUG: Cannot find suitable insertion point, no BODY, HTML or any tag, ABORTING", FL); 
		}

		if (p) {
			/** Our code below is expecting an insertion point, so we move it to the other side of the BODY tag. **/
			while ((p)&&(*p)&&(*p != '>')) p++;
		}
	} else {
		...
		if (glb.pretext_insert==1) {
			if (*p == '>') {
				*p = '\0';
				...
				DAM LOGGER_log("%s:%d:AM_insert_HTML_disclaimer_into_buffer:DEBUG: Composing HTML + PRETEXT disclaimer", FL);
...
int AM_insert_disclaimer_into_segment( FFGET_FILE *f, FILE *newf, struct AM_disclaimer_details *dd )
{
		...
		)
	{
		...
		if (glb.pretext_insert == 0) { 
			/* Process we go through if we're doing a disclaimer to get to 
			 * the bottom of the MIME segment.
			 *
			 * If we're doing a PRETEXT we'll actually jump straight to encoding
			 * and inserting the disclaimer text 
			 */
		...
		if (dd->content_encoding == _CTRANS_ENCODING_QP)
		{
			...
			DAM LOGGER_log("%s:%d:AM_insert_disclaimer_into_segment:DEBUG: Starting insertion of TEXT disclaimer into QUOTEDPRINTABLE section of text", FL);
...
Errors:
------------------------------------------------------------------------*/
int AM_filename_fix( char *line, size_t line_size, char *removed_prefix, int removed_count )
{
	char *pos;
...
int AM_alter_header( char *filename, char *header, char *change, int change_mode )
{
	...
	while (FFGET_fgets(line, sizeof(line),&f))
	{
		...
		if ((main_headers_complete == 0)&&(header_written == 0))
		{
			...
			snprintf( low_line, sizeof(line), "%s", line );
...
/*-----------------------------------------------------------------\
  Date Code:	: 20081205-144812
  Function Name	: AM_attachment_insert
  Returns Type	: int
  ----Parameter List
  1. char *fname, attachment filename to be inserted
  2.  char *insert_as_fname, use this filename rather than fname for the headers
  3.  char *cid, content ID, if NULL, random is generated
  4.  int encoding_format, encoding to use for the file, if 0, BASE64 is used
  5.  char *headers , headers for the new content, used if not NULL, will override insert_as_fname, cid.
  ------------------
  Exit Codes	: 
  0  success
  -1 error
...
\------------------------------------------------------------------*/
int AM_attachment_insert( char *fname, char *insert_as_fname, char *cid, int encoding_format, char *headers )
{
	return 0;
}
//-----------------------------------END----
